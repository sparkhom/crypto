...
def challenge29(p):
    prefix_mac = binascii.unhexlify(sha1(key + p))
    h = struct.unpack(">5I", prefix_mac)
    print(h)
    correct_mac = binascii.unhexlify(sha1(key + p + md_padding(key + p) + b';admin=true'))
    ...
    for i in range(64):
        ...
        forged_mac = binascii.unhexlify(sha1(b';admin=true', h[0], h[1], h[2], h[3], h[4], i + len(p + md_padding(b'\x00'*i + p) + b';admin=true')))
        if forged_mac == correct_mac:
            return forged_mac

if __name__ == '__main__':
    logging.basicConfig(stream=sys.stderr, level=logging.DEBUG)
    print(challenge29(b'comment1=cooking%20MCs;userdata=foo;comment2=%20like%20a%20pound%20of%20bacon'))
