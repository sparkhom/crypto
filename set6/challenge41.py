...
def challenge41():
    pub, priv = get_keys()
    c = encrypt_rsa(b'whipple it', pub[0], pub[1])
    s = int.from_bytes(os.urandom(4), byteorder='big') % pub[1] 
    cprime = (pow(s, pub[0], pub[1]) * c) % pub[1]
    pprime = decrypt_rsa(cprime, priv[0], priv[1])

    p = int.from_bytes(pprime, byteorder='big') * invmod(s, pub[1]) % pub[1]
